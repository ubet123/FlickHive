...
const FavouritesList = () => {
 ...
 // @ts-expect-error: TS1234 because the library definition is wrong
  ...
  const favoriteMovies = movieLists?.filter((movie) => favorites.includes(movie.id));
  const favoriteTvShows = tvShows?.filter((tv) => favorites.includes(tv.id));

  if (loading) {
    return (
      <div className="flex justify-center items-center mt-0 h-screen">
        <button
          className="inline-block w-60 h-20 rounded-full bg-green-500 text-neutral-50 shadow-[0_4px_9px_-4px_rgba(51,45,45,0.7)] hover:bg-green-600 hover:shadow-[0_8px_9px_-4px_rgba(51,45,45,0.2),0_4px_18px_0_rgba(51,45,45,0.1)] focus:bg-green-800 focus:shadow-[0_8px_9px_-4px_rgba(51,45,45,0.2),0_4px_18px_0_rgba(51,45,45,0.1)] px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal transition duration-150 ease-in-out focus:outline-none focus:ring-0"
          type="button"
        >
          <div
            role="status"
  ...
  return (
    <>
      <div className={`p-3 pt-6 ${isDarkMode ? "bg-gray-900 text-white" : "bg-blue-50 text-black"} min-h-screen`}>
  ...
  {/*Favorite TVshows section*/}

  <div className="px-8 mt-10">
          <h2 className="text-3xl font-semibold mb-6">TvShows</h2>
          <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-6">
            {favoriteTvShows?.length > 0 ? (
              ...
              <div className="col-span-full text-center text-gray-500">
                No favorite TvShows to display.
              </div>
            )}
          </div>
        </div>
